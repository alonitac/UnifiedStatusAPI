...
In this exercise, you are going to design and implement a unified API for availability status check of public services.
...
- AWS Service Health Dashboard API - https://status.aws.amazon.com
...
Here is the higher level design of the system you'll build:
...
The service runs continuously in the background, waking up at predetermined intervals to retrieve the status of services using the public services API (in the above diagram: GitHub, Facebook, Azure). 
...
Below you are provided with a detailed data scheme.
...
Every target (e.g. GitHub, Facebook, Azure) should be implemented as a Python class. 
The base class for all targets is `Target`, then there are 3 types of targets that inherit from `Target`:

- `HTMLTarget` - which retrieves data using HTML scraping techniques.
- `RestTarget` - which retrieves data by communicating with a RESTful API.
- `RSSTarget` - which retrieves data by subscribing to RSS feed.

The next level are the actual implementation of each target. In the above diagram there are one HTML target class for GitLab availability status data, two target classes for Zoom and GitHub, and one RSS target type for GCP. 
...
But ensure that the **data scheme** and **class structure** remain the same as described here.

You should collect availability status data from **at least 2 different targets**, to your choice.
You can either implement the targets based on RESTful, RSS, or dirt your hand, and scrape an HTML page.  

Under `targets/github.py` you'll find an example of an implemented target. 

The below list describe the endpoint that the Unified API service should support:
...
8. Click **Explore**
9. In the exploration panel, build a graph of the monitored targets over time.   
